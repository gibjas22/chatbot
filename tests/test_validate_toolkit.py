...
class SecretScanTests(unittest.TestCase):
    ...
    def test_a_real_pem_header_is_still_caught(self):
        text = "-----BEGIN RSA PRIVATE KEY-----\nMIIEow...\n"
        self.assertTrue(self.scan_text(text))

    def test_documenting_a_pem_search_is_not_a_credential(self):
        # The Strix skill teaches `git log -S 'BEGIN PRIVATE KEY'`. Writing that
        # down is not the same as committing a key, and the scanner must tell the
        # two apart or it cries wolf on its own documentation.
        text = "Run: git log -p --all -S 'BEGIN PRIVATE KEY' -- .\n"
